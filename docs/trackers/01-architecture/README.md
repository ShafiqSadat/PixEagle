...
Recovery algorithms remain provider-appropriate behind that shared contract.
Classic visual trackers share one bounded
[local-to-global detector recovery path](classic-recovery.md), while Smart
detector association and external gimbal providers retain their own valid loss
models. They all share the requirement that only a current measured state may
drive normal pursuit;
